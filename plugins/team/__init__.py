...
async def _(event: GroupMessageEvent, matcher: Matcher, args: Message = CommandArg()):
    ...
    if (
        user_id in team_manager[group_id].teams
        or user_id in team_manager[group_id].user_info
    ):
...
async def _(
    ...
    bot: Bot,
    name: str = ArgPlainText("new_team_name"),
    ...
    team_manager[group_id].new_team(
        name, user_id, user_info["nickname"], time_type_, bot, group_id
    )
...
async def _(
    event: GroupMessageEvent, bot: Bot, team_pass: str = ArgPlainText("join_team_name")
):
    ...
    if team_pass in team_manager[group_id].teams:
        ...
        await join_team.finish(
            Message()
            .append(
                f"加入队伍 {team_manager[group_id].teams[team_pass].team_name}成功了\n"
            )
...
force_del_team = plugin.cmd_as_group(
    cmd="强制结束", docs="强制结束一个队伍", permission=ADMIN
)
