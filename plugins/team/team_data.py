...
time_type = {"0": 0, "1": 5, "2": 10, "3": 15, "4": 20, "5": 30, "6": 45, "7": 60}
scheduler = plugin.scheduler_jobs()
...
class TeamManager:
    ...
    def new_team(self, team_name, user_id, user_name, t_type, bot, group_id):
        ...
        if t_type != "0":
            scheduler.add_job(
                self.show_info,
                trigger="interval",
                name=f"{self.group_id}-{user_id}",
                minutes=time_type[t_type],
                misfire_grace_time=15,
                args=[user_id, bot, group_id],
            )
        else:
            self.show_info(user_id, bot, group_id)
    ...
    def del_team(self, team_id):
        if scheduler.has_job(f"{self.group_id}-{team_id}"):
